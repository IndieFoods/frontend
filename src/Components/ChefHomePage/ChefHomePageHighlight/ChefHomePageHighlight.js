...
function ChefHomePageHighlight() {
    ...
    const highlightList = highlights.map((highlight, index) => {
        return (
            <ChefHomePageHighlightIndividual wrapperClass={styles.HighlightInstance} highlightTitle={highlight} highlightNumber={highlightData[index]} />
        )
    });

    return (
        <div className={styles.ChefHomePageHighlightContainer}>
            {highlightList}
        </div>
    )
}
